...
Just like in pretty much every other programming language that has classes, a Ruby class is a kind of template that objects can be based on.  I want lots of bingo cards, so now I'll take the code from part one that makes one card, use it to make a @BingoCard@ class, and then I can make lots of bingo cards.
...
Bingo cards that look pretty, with all baby pictures on, is a good job for HTML.  We can just put image tags with different img URLs into a HTML table -- so this card maker thingy should probably deal with image URLs instead of numbers.  Next, I'll change the code to put strings (that could be filenames or URLs or whatever) into the squares instead of numbers.  We can deal with the HTML layout later.
...
See the @[*'a'..'z']@?  There's a few interesting things here.  Firstly, Ruby understands _ranges_.  @1..10@ is a range that starts at @1@ and goes to @10@.  @'a'..'z'@ is a range that goes from @a@ to @z@.

Secondly, you can put an asterisk @*@ in front of a @Range@ to turn it into a list.  When it's used this way an asterisk is called a 'splat', and it's pretty weird.
...
Like a perfect girlfriend, Ruby wants you to be happy, and in this case, the @Array@ class aready has exactly the method we want: @delete_at@, which deletes an item from the array while also returning the deleted item.  So we can drop in a @delete_at@ like this:
