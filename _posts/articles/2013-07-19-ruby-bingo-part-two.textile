...
Notice that each card has its own version of the @@cards@ data structure.  Yay objects.  Also notice that it's slightly awkward to say @my_card.print_card@, and your Ruby code should be beautiful and graceful, like leaping gazelles silhouetted against the sunset as a light breeze stirs the long grass of the veldt.  So, I'll rename the @print_card@ method to just @show@ (I can't easily use @print@ because of the existing @print@ method).
...
<aside class="note language ruby" title="splat">
  Most Ruby programmers only use splat to handle method arguments, where it's great for writing methods that take varying
</aside>
